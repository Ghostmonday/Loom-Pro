...
Status: partial — happy path + post-sprint merge pipeline wired; failed phase pending
...
def _assert_greenfield_blueprint(payload: dict[str, Any]) -> None:
    if payload.get("blueprint_mode") != "intent":
        raise RuntimeError("greenfield sessions must freeze an intent blueprint")
    paths = [
        str(path)
    ...
    if len(paths) != len(set(paths)):
        raise RuntimeError("greenfield intent blueprint emitted duplicate allowed paths")
    if any("tiny_service" in path for path in paths):
        raise RuntimeError("greenfield intent blueprint leaked tiny_service template paths")
...
class OrchestrateSessionStore:
    ...
    def prepare(
        ...
        *,
        ...
        # GAIJINN: greenfield → intent_blueprint; else graph plan. Scan/analyze always.
        intent = intent.strip()
        if not intent:
    ...
    def advance_phase(self, session_id: str) -> SessionSnapshot:
        session_root = self._resolve(session_id)
        meta_path = session_root / ".gaijinn" / "session.json"
        meta = json.loads(meta_path.read_text(encoding="utf-8"))
        phases = normalize_phases(meta.get("phases"))
        ...
        message = f"{PHASE_LABELS[next_phase]} phase queued — blueprint stub"
        intent_context = "Run acceptance tests against merged deliverable"
        if next_phase == "frontend":
            intent_context = "Build frontend from backend merge summary"
        elif next_phase == "backend":
            intent_context = "Build backend from loaded frontend context"
        ...
        meta.update(
            {
                ...
                "phase_intent_context": intent_context,
                "workers_ready": 0,
                "selected_swarm": None,
                "swarm_warning": None,
                "idle_agents": 0,
    ...
    def _resolve(self, session_id: str) -> Path:
        ...
        if candidate.exists():
            self._active[session_id] = candidate
            return candidate
