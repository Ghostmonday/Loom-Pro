...
def run_smoke_test():
    ...
    for i in range(20):
        time.sleep(0.5)
        try:
            with urllib.request.urlopen("http://127.0.0.1:8080/api/v1/health", timeout=1.0) as conn:
                if conn.status == 200:
                    server_ready = True
                    break
        except Exception:
    ...
    try:
        ...
        req = urllib.request.Request("http://127.0.0.1:8080/vision_canvas/index.html")
        try:
        ...
        create_payload = {
            "prompt": "Build a secure audit ledger with auth",
            "tier": "paid"
        }
        create_headers = {
        ...
        assert new_version > blueprint_version, f"Expected version to advance from {blueprint_version}, got {new_version}"
        ...
        if new_question:
            ...
            assert new_question["question_id"] != question_id, "Expected next question ID to differ from prior question ID"
