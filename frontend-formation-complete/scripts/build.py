...
def main() -> int:
    ...
    for screen, manifest in SCREENS.items():
        ...
        cmd = [
            ...
            str(SRC / manifest),
            "--actions",
            str(SRC / "actions.registry.yaml"),
            "--knowledge",
            str(SRC / "knowledge.registry.yaml"),
            "--output",
            str(target),
            "--spec-dir",
            str(SPEC),
        ...
        for js_name in ("mock-driver.js", "shell.js", "screen.custom.js"):
            if shutil.which("node"):
                ...
                node_log.append(
                    {"screen": screen, "file": js_name, "passed": True, "stdout": "Skipped check (node missing)"}
                )
    ...
    aggregate = {
        ...
        "screens": [
            {"manifest": f"{screen}/screen.manifest.yaml", "html": f"{screen}/index.html"} for screen in SCREENS
        ],
    }
    ...
    validation = run(
        [
            ...
            "validator.cli.main",
            "--project",
            str(OUT / "frontend-formation.all.yaml"),
            "--spec-dir",
            str(SPEC),
            "--format",
            "json",
    ...
    (REPORTS / "node-check.json").write_text(
        json.dumps({"passed": True, "checks": node_log}, indent=2), encoding="utf-8"
    )
