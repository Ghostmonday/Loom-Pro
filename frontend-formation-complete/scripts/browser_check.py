...
def semantic_snapshot(page):
    return page.eval_on_selector_all('[data-classification]', '''nodes => nodes.map(n => ({
      id: n.id,
...
def main() -> int:
    ...
    with sync_playwright() as pw:
        ...
        def handle_post_session(route):
            route.fulfill(
                ...
                json={
                    ...
                    "latest_analysis": {
                        "rationale": "Prompt accepted; evidence-state analysis initialized."
                    }
                }
            )
        ...
        results['checks'].append({'name': 'terminal rejects swarm before prepare', 'passed': rejection.startswith('[rejected]')})
        ...
        results['checks'].append({'name': 'vision contract metadata stable', 'passed': before == semantic_snapshot(page)})
        ...
        results['checks'].append({'name': 'command action-specific progression', 'passed': 'prepared=true' in page.locator('#command-approval-display').inner_text()})
        results['checks'].append({'name': 'command contract metadata stable', 'passed': before == semantic_snapshot(page)})

        load_screen(page, 'terminal')
        before = semantic_snapshot(page)
        for control in ('terminal-assign-swarm-btn','terminal-deploy-btn','terminal-poll-grid-btn','terminal-merge-btn','terminal-poll-merge-btn','terminal-download-btn','terminal-diff-btn'):
            activate(page, control, 'terminal-feedback')
        results['checks'].append({'name': 'terminal merge completes cleanly', 'passed': 'blocked=0' in page.locator('#terminal-merge-display').inner_text() and 'ready=true' in page.locator('#terminal-deliverable-display').inner_text()})
        results['checks'].append({'name': 'terminal contract metadata stable', 'passed': before == semantic_snapshot(page)})

        load_screen(page, 'deliverable_launch')
        before = semantic_snapshot(page)
        for control in ('deliverable-detect-btn','deliverable-present-btn','deliverable-open-browser-btn','deliverable-download-btn','deliverable-diff-btn','deliverable-register-btn'):
            activate(page, control, 'deliverable-feedback')
        results['checks'].append({'name': 'deliverable launch and lineage', 'passed': 'state=succeeded' in page.locator('#deliverable-run-display').inner_text() and 'registered=true' in page.locator('#deliverable-lineage-display').inner_text()})
        results['checks'].append({'name': 'deliverable contract metadata stable', 'passed': before == semantic_snapshot(page)})

        # Independent continuation branch.
        load_screen(page, 'continuation')
        before = semantic_snapshot(page)
        for control in ('continuation-attach-btn','continuation-resume-btn','continuation-kind-btn','continuation-bootstrap-btn','continuation-mode-btn','continuation-answer-btn','continuation-scope-btn','continuation-confirm-handoff-btn','continuation-accept-handoff-btn'):
            activate(page, control, 'continuation-feedback')
        results['checks'].append({'name': 'continuation reaches handed off', 'passed': 'accepted=true' in page.locator('#continuation-handoff-display').inner_text()})
        results['checks'].append({'name': 'continuation contract metadata stable', 'passed': before == semantic_snapshot(page)})

        browser.close()

    results['passed'] = all(check['passed'] for check in results['checks']) and not results['console_errors'] and not results['page_errors']
