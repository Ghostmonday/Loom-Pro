    ...
    async dispatch(actionId) {
      ...
      switch (actionId) {
        case "intake.start_session": {
          try {
            const apiKey = globalThis.localStorage?.getItem("loom.api_key") || globalThis.LoomConfig?.apiKey || "";
            const base = (window.location.origin && window.location.origin !== "null" && !window.location.origin.startsWith("about"))
              ? window.location.origin
              : "http://localhost:8000";
            const res = await fetch(`${base}/api/v1/intent-forge/sessions`, {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
                "X-User-Id": state.vision.user_id || "user",
                "X-Loom-Api-Key": apiKey
              },
              body: JSON.stringify({
                prompt: state.vision.prompt,
                tier: "paid"
              })
            });
            if (!res.ok) {
            ...
            state.vision.session_id = data.session_id;
            state.vision.session_status = data.session_status;
            state.vision.current_question = data.current_question ?? null;
            state.vision.readiness = data.latest_analysis?.readiness ?? {
              score: 0,
              blocking_count: 0,
            ...
            state.vision.blueprint_version = data.blueprint_version;
            
            if (data.latest_analysis && data.latest_analysis.rationale) {
              state.vision.understanding = data.latest_analysis.rationale;
            } else {
            ...
            throw new Error(`API_FAILURE: ${err.message}`);
          }
        }
        ...
        case "question.submit_answer": {
          ...
          try {
            ...
            const questionId = state.vision.current_question?.question_id || "";
            const answerVal = document.getElementById("vision-answer-input")?.value || "";

            const res = await fetch(`${base}/api/v1/intent-forge/sessions/${state.vision.session_id}/answers`, {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
                "X-User-Id": state.vision.user_id || "user",
              ...
              body: JSON.stringify({
                ...
                answer: answerVal,
                expected_blueprint_version: state.vision.blueprint_version
              })
            });
            if (!res.ok) {
              const errData = await res.json().catch(() => ({}));
            ...
            if (data.latest_analysis && data.latest_analysis.rationale) {
              ...
              state.vision.understanding = "Answer accepted; evidence-state reanalysis complete.";
            }

            const inputEl = document.getElementById("vision-answer-input");
        ...
        case "question.revise_answer": {
          ...
          const currentScore = (typeof state.vision.readiness === "object" && state.vision.readiness) ? state.vision.readiness.score : state.vision.readiness;
          const newScore = Math.max(0.5, currentScore - 0.05);
          if (typeof state.vision.readiness === "object" && state.vision.readiness) {
            state.vision.readiness.score = newScore;
          } else {
            state.vision.readiness = newScore;
          }
          saveState(state);
        ...
        case "handoff.confirm": {
          ...
          const score = (typeof state.vision.readiness === "object" && state.vision.readiness) ? state.vision.readiness.score : state.vision.readiness;
          requireGate(state.vision.session_status === "FINALIZED" && score >= 0.8, actionId, "GATE_REFUSAL: readiness and FINALIZED status are required before confirmation.", ["loom.vision.handoff_state"]);
  ...
  globalThis.LoomProjectionDriver = {
    project(contractPath, result, targetElement) {
      ...
      const renders = {
        ...
        "loom.vision.current_question": () => {
          const q = state.vision.current_question;
          const txt = (typeof q === "object" && q) ? q.text : (q || "No active question.");
          return `[QUESTION] ${txt}`;
        },
        "loom.vision.understanding": () => `[UNDERSTANDING] ${state.vision.understanding}`,
        "loom.vision.readiness": () => {
          const r = state.vision.readiness;
          const score = (typeof r === "object" && r) ? r.score : r;
          return `[READINESS] ${percent(score)} | status=${state.vision.session_status}`;
        },
