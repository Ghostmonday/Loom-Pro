        ...
        if el.classification != expected_class:
            violations.append(
                Violation(
                    ...
                    el.ref,
                    (
                        f"DOM classification '{el.classification}' does not match "
                        f"manifest classification '{expected_class}'."
                    ),
                    RULE_NAME,
                    document.path,
                )
            ...
            if el.feedback_target != manifest_el.get("feedback_target"):
                violations.append(
                    Violation(
                        ...
                        el.ref,
                        (
                            f"DOM feedback target '{el.feedback_target}' does not match "
                            f"manifest feedback target '{manifest_el.get('feedback_target')}'."
                        ),
                        RULE_NAME,
                        document.path,
                    )
            ...
            if el.contract_path != manifest_el.get("contract_path"):
                violations.append(
                    Violation(
                        ...
                        el.ref,
                        (
                            f"DOM contract path '{el.contract_path}' does not match "
                            f"manifest contract path '{manifest_el.get('contract_path')}'."
                        ),
                        RULE_NAME,
                        document.path,
                    )
