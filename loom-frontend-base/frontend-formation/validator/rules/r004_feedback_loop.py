    ...
    for el in document.elements:
        ...
        if not target:
            ...
            if target_dom is None:
                violations.append(Violation(
                    "FFM-R004-02", "ERROR", el.ref,
                    f"Feedback target '{target}' does not exist in the DOM.",
                    RULE_NAME, document.path,
                ))
            elif target_dom.classification != "display":
                violations.append(Violation(
                    "FFM-R004-06", "ERROR", el.ref,
                    f"Feedback target '{target}' is classified '{target_dom.classification}' in the DOM, not display.",
                    RULE_NAME, document.path,
                ))
