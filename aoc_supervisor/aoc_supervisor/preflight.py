...
def resolve_preflight_project_root(
    ...
    *,
    ...
    raise PreflightResolutionError(
        f"unable to resolve project root for session {session_id!r}; provide project_path or set GAIJINN_PROJECT_ROOT",
    )
...
def run_preflight_check(
    *,
    ...
    if not blueprint:
        rejection_reasons.append("blueprint_missing")
    if not worker_dir.is_dir():
        rejection_reasons.append("worker_not_found")
    if not manifest_path.is_file():
        rejection_reasons.append("worker_manifest_missing")
    ...
    if not isinstance(worker_record, dict):
        rejection_reasons.append("worker_validation_missing")
    else:
        if "passed" not in worker_record:
            rejection_reasons.append("worker_validation_incomplete")
        else:
            worker_passed = bool(worker_record.get("passed"))
            if worker_passed is not True:
                rejection_reasons.append("worker_validation_not_passed")

    allow_merge = not rejection_reasons
