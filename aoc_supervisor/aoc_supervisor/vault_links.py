...
class VaultLinkResolver:
    ...
    def _coerce_links(self, links_or_content: str | list[CrossVaultLink]) -> list[CrossVaultLink]:
        if isinstance(links_or_content, str):
            return self.parse_links(links_or_content)
        return links_or_content
    ...
    def broken_links(self, links_or_content: str | list[CrossVaultLink]) -> list[CrossVaultLink]:
        ...
        links = self._coerce_links(links_or_content)
    ...
    def report(self, links_or_content: str | list[CrossVaultLink]) -> str:
        ...
        total = len(resolved)
        broken = [link for link in resolved if link.is_broken]
        ok = [link for link in resolved if not link.is_broken]
